...
class RemoteSimulator:
  ...
  def _handleException(self, result):
    ...
    if self.verbose and not result.isSuccess():
      print(
          result.getRelatedCommand().getName()
          + " failed: "
          + result.getMessage()
      )
  ...
  def _checkForbiddenPost(self, cmd):
    if cmd.getName() == "Start":
      raise UsageError(
          "You cannot send a Start command. Use RemoteSimulator.start()"
          " instead."
      )
  ...
  def pushRouteEcef(self, speed, ecef):
    ...
    if speed <= 0:
      raise UsageError(
          "A route node must have a speed limit greater than zero."
      )
  ...
  def pushIntTxTrackEcef(self, elapsedTime, ecef, id):
    ...
    self._postCommand(
        PushIntTxTrackEcef(elapsedTime, ecef.x, ecef.y, ecef.z, id)
    )
    return True
  ...
  def endIntTxTrackDefinition(self, id):
    ...
    if self.verbose:
      print(
          "End transmitter track contains "
          + str(trackResult.count())
          + " nodes."
      )
...
class RemoteSpooferSimulator(RemoteSimulator):
  ...
  def connect(self, ip="localhost", id=1, failIfApiVersionMismatch=False):
    RemoteSimulator.connect(
        self, ip, spooferInstance(id), failIfApiVersionMismatch
    )
