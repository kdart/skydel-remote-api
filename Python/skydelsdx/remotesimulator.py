...
from .exceptions import CommandException, OperationalError, UsageError
...
class RemoteSimulator:
    ...
    def connect(self, hostname="localhost", id=0, failIfApiVersionMismatch=False):
        if self.isConnected():
            raise UsageError("Cannot connect. Already connected. Disconnect first.")

        port = 4820 + id
        if self.verbose:
            print("Connecting to %s on port %s..." % (hostname, port))
        self.client = ClientCmd()
        self.client.connect(hostname, port)

        self._serverApiVersion = self.client.getServerApiVersion()
        if ApiVersion != self._serverApiVersion:
            msg = ("Client Api Version (" + str(ApiVersion) + ") != Server Api Version (" +
        ...
        self.hil = ClientHil()
        self.hil.open((self.client.server_address, hilPort))
    ...
    def start(self):
        ...
        if self.verbose:
            ...
            if (self._callCommand(
                    GetSimulatorState()).subStateId() == SimulatorSubState.Started_HILSync):
    ...
    def stop(self, timestamp=None):
        ...
        if timestamp is not None and self.verbose:
    ...
    def _checkForbiddenPost(self, cmd):
        if cmd.getName() == "Start":
            raise UsageError("You cannot send a Start command. Use RemoteSimulator.start()"
                             " instead.")
    ...
    def _checkForbiddenCall(self, cmd):
        ...
        if cmd.getName() == "PushRouteEcef":
            raise UsageError("You cannot call a PushRouteEcef command. Post it or use"
                             " RemoteSimulator.pushRouteLla() or RemoteSimulator.pushRouteEcef()"
                             " instead.")
        if cmd.getName() == "PushTrackEcef":
            raise UsageError("You cannot call a PushTrackEcef command. Post it or use"
                             " RemoteSimulator.pushTrackEcef() or RemoteSimulator.pushTrackLla()"
                             " instead.")
        if cmd.getName() == "PushTrackEcefNed":
            raise UsageError("You cannot call a PushTrackEcefNed command. Post it or use"
                             " RemoteSimulator.pushTrackEcefNed() or"
                             " RemoteSimulator.pushTrackLlaNed() instead.")
        if cmd.getName() == "PushIntTxTrackEcef":
            raise UsageError("You cannot call a PushIntTxTrackEcef command. Post it or use"
                             " RemoteSimulator.pushIntTxTrackEcef() or"
                             " RemoteSimulator.pushIntTxTrackLla() instead.")
        if cmd.getName() == "PushIntTxTrackEcefNed":
            raise UsageError("You cannot call a PushIntTxTrackEcefNed command. Post it or use"
                             " RemoteSimulator.pushIntTxTrackEcefNed() or"
                             " RemoteSimulator.pushIntTxTrackLlaNed() instead.")
    ...
    def checkIfStreaming(self):
        ...
        if self.exception_on_error:
            raise OperationalError(errorMsg)
        if self.verbose:
            print(errorMsg)
        return False
    ...
    def _checkHil(self, pos, elapsedTime):
        if self.hil is None:
            raise UsageError("Cannot send positions to simulator because you are not connected.")
    ...
    def _checkConnect(self):
        if not self.isConnected():
            raise UsageError("You are not connected.")
    ...
    def pushRouteEcef(self, speed, ecef):
        ...
        if not self.beginRoute:
            raise UsageError("You must call beginRouteDefinition first.")

        if speed <= 0:
            raise UsageError("A route node must have a speed limit greater than zero.")
    ...
    def pushTrackEcef(self, elapsedTime, ecef):
        ...
        if not self.beginTrack:
            raise UsageError("You must call beginTrackDefinition first.")
    ...
    def pushIntTxTrackEcef(self, elapsedTime, ecef, id):
        ...
        if not self.beginIntTxTrack[id]:
            raise UsageError("You must call beginIntTxTrackDefinition first.")
    ...
    def _postCommand(self, cmd, timestamp=None):
        ...
        if deprecated and (self.deprecatedMessageMode == DeprecatedMessageMode.ALL or
                           (self.deprecatedMessageMode == DeprecatedMessageMode.LATCH and
                            cmd.getName() not in self.latchDeprecated)):
